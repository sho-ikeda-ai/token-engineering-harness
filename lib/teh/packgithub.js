...
// Machine-specific notes (e.g. cross-agent messaging hooks injected by local
// automation into every repo under C:\dev) can land in an otherwise-public
// file. Rather than excluding the whole file, wrap the note in these markers;
// the scanner ignores fenced content, and scripts/strip-local-only.js removes
// it for real before publishing (see docs/publishing.md).
const LOCAL_ONLY_START = '<!-- LOCAL-ONLY:START';
const LOCAL_ONLY_END = '<!-- LOCAL-ONLY:END -->';

function stripLocalOnlyBlocks(text) {
  const lines = text.split(/\r?\n/);
  const out = [];
  let inBlock = false;
  for (const line of lines) {
    if (!inBlock && line.includes(LOCAL_ONLY_START)) { inBlock = true; continue; }
    if (inBlock) {
      if (line.includes(LOCAL_ONLY_END)) inBlock = false;
      continue;
    }
    out.push(line);
  }
...
function scan() {
  ...
  for (const f of F.listFilesRecursive(root)) {
    ...
    text = stripLocalOnlyBlocks(text);
    const lines = text.split(/\r?\n/);
...
module.exports = { run, scan, stripLocalOnlyBlocks, SECRET_RULES, TRACE_RULES };
