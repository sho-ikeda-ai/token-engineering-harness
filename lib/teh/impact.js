...
function run(args) {
  ...
  if (args.includes('--png')) {
    renderPng(dataPath, outDir);
  }
}

function renderPng(dataPath, outDir) {
  const script = path.join(__dirname, '..', '..', 'scripts', 'impact_png.py');
  const r = spawnSync('python', [script, dataPath, outDir], { encoding: 'utf8' });
  if (r.error || r.status !== 0) {
    console.log('warning: --png rendering skipped (python/Pillow unavailable or failed)');
    return;
  }
  console.log('wrote ' + path.join(outDir, 'teh-impact-wide.png'));
  console.log('wrote ' + path.join(outDir, 'teh-impact-story.png'));
}
